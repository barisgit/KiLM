...
<Aside type="note">
  **Note:** Both **Creators** and **Consumers** may need to understand the
  configuration, but **Creators** are more likely to directly manage the library
  entries within `config.yaml`.
</Aside>
...
<Aside type="caution">
  **Important:** KiLM creates timestamped backups of KiCad's configuration files
  (e.g., `fp-lib-table.backup.YYYYMMDD_HHMMSS`,
  `sym-lib-table.backup.YYYYMMDD_HHMMSS`,
  `kicad_common.json.backup.YYYYMMDD_HHMMSS`) in the *same* directory as the
